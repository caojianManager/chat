...
class ChatHomeViewController: UITabBarController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        let currentUserData = ChatUserDefaultUtil.getCommonUserDefault(key: currentUser)
        guard let currentUserStr = String(data: currentUserData, encoding: String.Encoding.utf8) else {
            return  }
        let currentUser = UserBase.getUserBase(currentUserStr)
        UserSessionManager.getInstance.setCurrentUser(user: currentUser)

        ChatUserInfoProcessor.initUserInfoDB(user: currentUser)
        ChatNetWork.getInstance.connect()
